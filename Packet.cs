class Packet
{
    ...
    public Packet(Type type, Player player)
    {
        this.type = type;
        bytes = new byte[8];
        BitConverter.GetBytes((int)type).CopyTo(bytes, 0);
        BitConverter.GetBytes(player.id).CopyTo(bytes, 4);
        protocol = (type == Type.MOVED||type==Type.PING) ? Protocol.UDP : Protocol.TCP;
    }
    public Packet(Type type, Player player, int a)
    {
        this.type = type;
        bytes = new byte[12];
        BitConverter.GetBytes((int)type).CopyTo(bytes, 0);
    ...
    public Packet(Type type, Player player, byte[] extraBytes)
    {
        this.type = type;
